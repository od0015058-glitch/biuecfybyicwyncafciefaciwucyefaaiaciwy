...
| `conversation_export.py` | **Stage-15-Step-E #1 (STARTED PR #119).** `.txt` exporter for a user's conversation buffer. `format_history_as_text(rows, user_handle)` renders the buffer with role labels + UTC timestamps + a header (`"Messages: {kept} (trimmed {dropped} oldest)"`) and oldest-first-truncates if the body exceeds the 1 MB cap so the output always fits Telegram's document size. `export_filename_for(telegram_id)` returns a stable `meowassist-history-<id>-<YYYY-MM-DD>.txt`. Wired through the memory-screen "📥 Export conversation" button (`memory_export_handler`). Reads via `Database.get_full_conversation` (full buffer with timestamps, NOT the live-context `get_recent_messages` which drops them). Button stays visible even when memory is OFF — the buffer is user-owned data and they should always be able to recover it. **Remains:** `.pdf` export, `/history` text-command alias, pagination for very large buffers, rate-limit the export button. |
| `user_stats.py` | **Stage-15-Step-E #2 (STARTED this PR).** Per-user spending digest. `format_user_stats(summary, lang)` renders the dict from `Database.get_user_spending_summary` as a Markdown digest: total calls, total spent, prompt/completion tokens, 7d / 30d windows, top-N model breakdown (capped at `DIGEST_TOP_MODELS=5`). Empty-state ("No AI calls yet") for users with zero usage rows. Defensive NaN-guarding via local `_safe_float` / `_safe_int` (mirrors `wallet_display`). Wired through the wallet-screen "📊 My usage stats" button (`hub_stats_handler`) and the `/stats` text-command alias (`cmd_stats`). **Remains:** per-day / per-week chart, CSV export, daily-breakdown sub-screen, locale-formatted timestamps for first/last call lines, `/admin_user_stats <id>` impersonation view that reuses the same digest. |
...
| **Stage-15-D** | Bug-fix sweep | shipped (6 bundled fixes across PRs #113–#118) |
| **Stage-15-E** | Future project suggestions (12 items) | STARTED — first slices in flight (E#1: PR #119; E#2: this PR) |
