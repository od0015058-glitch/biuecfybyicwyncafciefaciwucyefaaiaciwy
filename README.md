...
- **FX-refresh interval editor** on `/admin/wallet-config`
  (Stage-15-Step-E #10b row 24) — DB-backed override for
  `FX_REFRESH_INTERVAL_SECONDS` (default 600 s / 10 min, range
  [60, 86 400] = 1 minute to 1 day). The USD→Toman refresher loop
  re-reads the interval on every tick so a saved override is live
  without a redeploy. Bundled bug fix: the FX refresher now
  pushes its resolved cadence into `bot_health.LOOP_CADENCES` via
  the new `fx_rates._sync_registered_cadence` helper, so retuning
  the cadence from the panel no longer leaves the panel forever
  showing the FX loop as "running late". Mirrors the row-21
  bot-health-alert fix.
...
- **Refund presets editor** on `/admin/refund-presets`
  (Stage-15-Step-E #10b row 28) — DB-backed override layer for the
  operator-curated list of common refund reasons (≤5 entries × ≤40
  chars each). Surfaces as a `<select name="reason_preset">` dropdown
  above the free-text reason field on the `/admin/transactions`
  refund form — picking a preset seeds the textarea via inline JS so
  the operator can still edit before submitting. Newline + pipe
  separator both accepted in the editor textarea so an operator can
  paste from a CSV-like source. Compile-time defaults: "Duplicate
  payment", "User-requested cancellation", "Stuck invoice — never
  credited", "Fraud / chargeback", "Bot or platform error". Saving
  an empty list hides the dropdown entirely (some deploys want the
  legacy textarea-only shape). Audit slug `refund_presets_update`
  carries before/after lists for forensics. Resolution order:
  `system_settings.REFUND_PRESETS` (JSON-encoded list) → env
  `REFUND_PRESETS` (newline / pipe-separated) → compile-time
  default. Bundled bug fix: new `_scrub_audit_meta` helper in
  `database.py` makes `record_admin_audit` AND
  `record_payment_status_transition` lossless across `Decimal`
  (every money-handling caller produces these from asyncpg numeric
  reads), `datetime` / `date` (every "X happened at" meta), and
  non-finite `float` (NaN / ±Infinity from a corrupted balance).
  Pre-fix, any of those silently crashed `json.dumps`, the bare
  `except` swallowed the row, and the audit feed forgot the event
  ever happened — exactly the wrong shape for an audit log whose
  entire job is "do not lose anything". Recursive walk over dicts
  / lists; `tuple` / `set` → list; `bytes` → utf-8; non-finite
  floats → JSON `null`; unknown types → tagged
  `<unscrubbable:Type:repr>` for grep-ability. Belt-and-suspenders:
...
| `fx_refresh_config.py` | **Stage-15-Step-E #10b row 24.** DB-backed override layer for `FX_REFRESH_INTERVAL_SECONDS`. Same shape as `model_discovery_config.py`: module-level cache, coercion validator (range [60, 86 400] = 1 min – 1 day), set/clear/get/refresh-from-DB helpers, resolution order (override → env → default 600 s / 10 min). Editor on `/admin/wallet-config`. The USD→Toman refresher loop in `fx_rates.refresh_usd_to_toman_loop` re-reads the interval on every tick so a saved override is live without a redeploy, and pushes the resolved cadence into `bot_health.LOOP_CADENCES` via `fx_rates._sync_registered_cadence` so the `/admin/control` panel's "stale threshold" tracks the loop's actual sleep duration (mirrors the row-21 bot-health-alert fix). |
| `refund_presets.py` | **Stage-15-Step-E #10b row 28.** DB-backed override layer for the operator-curated refund-reason presets. Same shape as the rest of the `*_config.py` modules but storing a JSON-encoded `list[str]` rather than a single scalar: module-level cache (`None` = no override, `[]` = explicit "hide the dropdown" choice, non-empty = active list), coercion (`coerce_refund_presets`) that strips / NUL-strips / case-insensitive-dedupes / length-caps each entry, parse helper (`parse_refund_presets_text`) that splits on newlines and pipes, JSON encode/decode round-trip via `encode_refund_presets_for_storage` / `_decode_stored_presets`, set/clear/get/refresh-from-DB helpers, resolution order (override → env `REFUND_PRESETS` → compile-time `DEFAULT_REFUND_PRESETS`). Bounds: ≤5 entries × ≤40 chars (worst-case JSON payload 216 chars fits the 255-char `system_settings.setting_value` column). Editor on `/admin/refund-presets`. Boot warm-up in `main.py`. Surfaces as a `<select name="reason_preset">` dropdown above the free-text reason field on the `/admin/transactions` refund form. |
...
| `abuse_detection.py` | **Stage-16 row 20.** Three independent abuse-detection layers wired into the chat hot-path: (1) `is_oversized` length cap (default 50 000 chars via `ABUSE_MAX_PROMPT_CHARS`, floor 1 000 so an env-typo can't silently disable), (2) `classify` regex scan for SQL-injection (`UNION SELECT`, `DROP TABLE`, `OR 1=1`, `; --`), XSS (`<script`, `javascript:`, `onerror=`), shell-injection (`$(curl`, `wget http`) and 200+-char repetition spam, (3) `SpendSpikeTracker` sliding-window per-user spend tracker with idempotent per-user latch. Each layer toggle-able via `ABUSE_DETECTION_ENABLED` (master), `ABUSE_PATTERNS_ENABLED`, `ABUSE_SPEND_SPIKE_ENABLED`. The classifier short-circuits in `process_chat` BEFORE `consume_chat_token`, returning the same `ai_local_rate_limited` reply for every non-OK classification so an attacker can't probe which rule fired. Spend-spike: `ai_engine.chat_with_model` calls `record_paid_spend` after each paid settlement; the chat handler drains the latch via `maybe_alert_spend_spike` after the AI reply is sent so an admin DM 5xx can't lose the user's reply. Per-admin fault isolation matches `model_discovery.notify_admins`. |
...
| `ai_feedback.py` | **Stage-16 row 19.** Per-reply 👍 / 👎 rating system. `build_feedback_keyboard(log_id)` emits a two-button inline keyboard (`fbp:<log_id>` for 👍, `fbn:<log_id>` for 👎) attached to the LAST chunk of every AI reply by `process_chat` and `process_photo` after the underlying `usage_logs` row is written; the callback handler in `handlers.py` decodes the slug, calls `db.record_usage_feedback` (which enforces a per-user owner check via `WHERE telegram_id = $2` so users can't rate someone else's reply, and first-tap-wins idempotency via `WHERE feedback IS NULL` so the second tap is a no-op even if the user changes their mind), surfaces a localised toast, and strips the keyboard. The alert layer aggregates per-model dissatisfaction over a trailing window (`AI_FEEDBACK_DISSATISFACTION_WINDOW_SECONDS`, default 1 h) and DMs admins once per `AI_FEEDBACK_ALERT_COOLDOWN_SECONDS` (default 1 h) when the negative rate crosses `AI_FEEDBACK_DISSATISFACTION_THRESHOLD_RATIO` (default 0.30) on at least `AI_FEEDBACK_MIN_SAMPLES` (default 10) rated rows. Per-admin fault isolation mirrors `model_discovery.notify_admins`. The loop is registered via `@register_loop("ai_feedback_alert", ...)` so its heartbeat is scraped at `/metrics` and the panel can detect a wedged loop. Master switches: `AI_FEEDBACK_ENABLED` (the keyboard itself, default true), `AI_FEEDBACK_ALERT_ENABLED` (the alert loop, default true). |
