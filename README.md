...
| `database.py` | asyncpg pool + every SQL query. Money methods use `SELECT … FOR UPDATE` inside connection-scoped transactions. `create_promo_code` / `create_gift_code` refuse non-finite `discount_amount` / `amount_usd` at the DB layer (PG `NUMERIC` accepts `'NaN'::numeric` and a stored NaN would brick the redeemer's wallet via `balance_usd + NaN`). |
