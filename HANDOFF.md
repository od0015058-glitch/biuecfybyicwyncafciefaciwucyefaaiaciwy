...
| # | Title | Priority | Status |
| --- | --- | --- | --- |
| **Stage-11-Step-A** | **Live USD↔IRR exchange rate ticker.** New module `fx_rates.py` with a background refresher (`refresh_usd_irr_loop`, default 10-min cadence, env `FX_REFRESH_INTERVAL_SECONDS`). In-memory cached `(rate, fetched_at)` plus DB row in a new `fx_rates_snapshot` table (migration 0010) so a process restart doesn't start from a cold cache. Source: default `nobitex.ir/api/market/stats` (free, high-volume, no auth) with env-overridable `FX_RATE_ENDPOINT` for easy swap. Same fault-tolerance pattern as Stage-10-Step-A `refresh_min_amounts_once`: on fetch failure keep the previous cached value rather than collapsing to a default (a silent collapse would gift $2 charges during a rate spike or refuse legitimate payments during a rate dip). Admin DM on >10% rate move (cadence-level, not within-session) — `FX_RATE_ALERT_THRESHOLD_PERCENT` env, default 10. Tests: fresh fetch, cache preservation on outage, stale-value staleness reporting, threshold-crossing alert. | P1 foundational | ⏳ pending |
