...
| 8 | **Stripe / Zarinpal payment gateway** — add conventional card payment options alongside crypto. Stripe for international, Zarinpal for Iranian cards (alternative to TetraPay). **STARTED — see "Stage-15-Step-E #8" section below for the Zarinpal v4 first-slice (create / verify / browser-redirect callback) + bundled `model_discovery` / `fx_rates` int-env import-time-crash + missing-floor bug fix. Stripe still pending.** | P2 product | High | Significant gateway integration work |
| 9 | **Bot monetization dashboard** — admin page showing revenue vs. OpenRouter cost, profit margin per model, break-even analysis. All data already exists in `usage_logs` + `transactions`. **STARTED — see "Stage-15-Step-E #9" section below for the `/admin/monetization` first-slice (windowed P&L + per-model profit margin) + bundled Zarinpal-not-in-`REFUNDABLE_GATEWAYS` / `TRANSACTIONS_GATEWAY_VALUES` / `_collect_ipn_health` regression fix.** | P2 product | Medium | High value for the operator to understand business health |
...
* **Telegram FSM integration.** The Zarinpal create / verify / callback triple is fully wired into `main.start_webhook_server` and exercised by 62 unit tests, but no Telegram FSM state currently routes a user to `zarinpal.create_order`. The next slice should add a "💳 پرداخت با زرین‌پال" button next to the existing TetraPay button in `handlers.py`, a Zarinpal counterpart to `tetrapay_topup_*` handlers (FSM state, amount entry, locked-rate snapshot, invoice render with "Open Zarinpal" inline button). Mirror the TetraPay flow exactly — same FSM transitions, same error / unreachable handling, same `tetrapay_unreachable` → `zarinpal_unreachable` string. Probably ~150 LoC of `handlers.py` + 4–5 new strings.
...
* **Stripe (the other half of row 8).** This entire PR is Zarinpal-only. Stripe is materially heavier: SCA (Strong Customer Authentication) flow, SetupIntent + PaymentIntent, multi-currency handling (the bot's wallet is USD-denominated; Stripe routes through `automatic_payment_methods` then needs a fee-aware credit), Stripe-signed webhook verification with the official `stripe` SDK signature header (per-event, not per-body). Probably ~700 LoC + 60+ tests. Should land as its own PR (Step-E #8b) so the Zarinpal slice can ship first and de-risk the integration shape.
...
* **Drop-counter visibility on `/admin/`.** `web_admin._collect_ipn_health` already renders TetraPay + NowPayments tiles. Adding a third tile for Zarinpal is a 5-line `for accessor in (..., zarinpal.get_zarinpal_drop_counters)` extension. *Shipped in PR-after-Step-E-#8 as the Step-E #9 bundled bug fix — see the next section.*
...
**Step-E #9 (Bot monetization dashboard) — STARTED in PR-after-Step-E-#8.**

Original spec (Step-E table row 9): "admin page showing revenue vs. OpenRouter cost, profit margin per model, break-even analysis. All data already exists in `usage_logs` + `transactions`." The premise is correct: every input we need is already on disk; we just don't render it. This slice ships the rendering layer — the data-pipeline / forecasting work stays for later.

What's shipped this PR — **the windowed P&L view** (the headline numbers + per-model breakdown; break-even analysis stays for the next slice once we have a per-row OpenRouter-cost backfill column to lean on):

* `Database.get_monetization_summary(*, window_days, top_models_limit)` — new aggregation query in `database.py`. Returns `revenue_usd` / `refunded_usd` / `promo_bonus_paid_usd` / `gross_charge_usd` / `openrouter_cost_usd` / `gross_profit_usd` plus per-call / per-user counts and a `per_model` list (top N models by `gross_charge_usd` with calls + gross + estimated OpenRouter cost + estimated profit per row). `window_days=None` is lifetime; bounded windows clip BOTH halves of the P&L (transactions side via `completed_at` / `refunded_at`, usage_logs side via `created_at`) so revenue and cost are apples-to-apples — a 30d revenue against a lifetime cost would have produced a nonsense gross-profit figure. `window_days` is clamped to `[1, MONETIZATION_WINDOW_DAYS_MAX]` (default cap 3650 days ≈ 10 years) so a misconfigured `?window=…` URL can't degrade the bounded-window code path into a full table scan that pretends to be bounded. `top_models_limit` is clamped to `[1, MONETIZATION_TOP_MODELS_LIMIT_MAX]` (default cap 50). Refunds use the dedicated `refunded_at` timestamp for windowed filtering (NOT `completed_at` — a 30d-old credit refunded yesterday must show up in the 7d refund tile, which `completed_at` would silently exclude). Per-model rows sort by `gross_charge_usd DESC, model_used ASC` — the tie-breaker on the model name keeps the table stable across page reloads (otherwise Postgres returns rows in physical-storage order on a tie and the table would flicker). Defensive: `pricing.get_markup` is read once at the top of the method (so a racing env-reload between the 4 SQL roundtrips can't leave the per-model rows inconsistent with the totals) and re-floored to 1.5 if the helper somehow returns 0 / -X / NaN (the helper's own guard already catches non-finite, but a future regression there would otherwise produce `inf` / `nan` in the OpenRouter-cost column).
* `web_admin.monetization` handler + route registration at `/admin/monetization`. Same defensive pattern as the main dashboard — missing DB or DB-error renders the empty shape with a warning banner instead of a 500. The handler resolves the `?window=` query param (`24h` / `7d` / `30d` / `90d` / `all`) via the public `MONETIZATION_WINDOW_OPTIONS` tuple; unknown / malformed values fall back to the 30d default rather than raising (a stale URL must not 500). Margin% is computed in the handler (not the DB layer) so a zero-revenue window renders an em-dash placeholder rather than `inf` / `nan` / a misleading `0%`.
* `templates/admin/monetization.html` — new Jinja2 template. Headline tiles (revenue / refunds / promo bonus paid / gross AI charge / OpenRouter cost / gross profit / margin %), per-model table, and a "How this is computed" footer that documents every filter so an operator can audit the math. The window selector along the top is a tab strip — clicking "Last 7 days" reloads the same URL with `?window=7d`. Sidebar nav in `_layout.html` gets a new "💰 Monetization" entry slotted between "📊 Dashboard" and "🎟️ Promo codes".
* 18 new test cases (10 in `test_database_queries.py`, 7 in `test_web_admin.py`, 1 update to the existing `test_dashboard_renders_ipn_health_*` triple) covering: full shape pin / revenue gateway-exclusion / refunds use refunded_at not completed_at / lifetime drops every INTERVAL filter / windowed binds the int instead of interpolating it / `window_days=0`/`-30` raises / top_models_limit clamps high+low / zero-revenue doesn't NaN / markup-fallback-to-1.5 defensive guard / per-model sort tie-breaker / route requires auth / 30d default window / each `?window=` slug routes to right `window_days` / unknown slug falls back to default / DB error renders zero shape / zero-revenue renders em-dash + no NaN/Inf token leaks / sidebar link is reachable. Suite goes 1658 → 1676 (+18 new, 0 modified).

What's NOT shipped (deliberately deferred to subsequent slices, surfacing here so the next AI knows where to start):

* **Per-row OpenRouter-cost column on `usage_logs`.** Currently the table only stores `cost_deducted_usd` (what we charged the user); the OpenRouter-cost column is reverse-derived as `cost_deducted_usd / get_markup()` at query time. If the operator changes `COST_MARKUP` mid-window, historical rows priced under the old markup are reported as if they were priced under the new one. The fix is a `0017_usage_logs_openrouter_cost.py` migration adding `openrouter_cost_usd DECIMAL(10, 6)` (nullable, backfill behind a feature flag) plus a `pricing.calculate_cost_async` change to log it. Probably ~50 LoC + ~10 tests + a ~30-min backfill pass on the production table.
* **Break-even analysis tile.** The Step-E table row literally lists "break-even analysis" as a deliverable. The minimal version: read the operator's monthly fixed-cost figure from a new `BOT_FIXED_COSTS_USD_MONTHLY` env var (server, IP, gateway fees, etc.) and surface it as a "you need $X/month in revenue to break even at the current margin %" tile. Probably ~30 LoC + a couple of tests. Held over because the math is sensitive to the markup-historical issue above — once OpenRouter cost is per-row authoritative, the break-even computation is meaningfully more accurate.
* **Daily / weekly time-series chart.** A small inline SVG sparkline showing revenue + cost + profit over the window, instead of just the headline scalar. Nice-to-have polish for trend spotting.
* **CSV export.** A `/admin/monetization.csv?window=30d` dump for the operator's accountant. Pure plumbing — a couple of `aiohttp.web.StreamResponse` lines + a test.
* **Per-user profitability drill-down.** Right now we show per-model. Per-user (especially "top 10 most profitable users" + "top 10 loss users" because someone's daily free-message allowance is being clipped without a top-up) would surface useful retention signals. The data is already in `usage_logs` (telegram_id) and `transactions` (telegram_id); just needs a second aggregation method + a sub-page.
* **Telegram FSM entry-point.** Step-E #9 is intentionally admin-only. There's no `/admin_monetization` Telegram command. If the operator wants to read the monetization dashboard from Telegram, a future slice could add a `/admin_pnl` admin command that DMs the headline numbers, mirroring the existing `/admin_metrics` shape. Low priority — the web dashboard is the canonical surface.

Bundled bug fix in this PR (real, found during the Step-E #9 code audit while reading `web_admin._collect_ipn_health` to understand how to plug a third gateway tile in): **Step-E #8 (PR #126) shipped the Zarinpal v4 gateway integration but did not extend three places where the gateway list is enumerated.** Three concrete production bugs landed silently:

1. `Database.REFUNDABLE_GATEWAYS = frozenset({"nowpayments", "tetrapay"})` — Zarinpal was never added. The admin refund flow at `/admin/transactions/<id>/refund` calls `db.refund_transaction`, which checks `tx_row["gateway"] not in self.REFUNDABLE_GATEWAYS` and returns `REFUND_REFUSAL_GATEWAY_NOT_REFUNDABLE`. Every Zarinpal SUCCESS row was therefore unrefundable via the panel, even though Zarinpal supports refunds (the operator could only reverse the credit by going off-ledger via `admin_adjust_balance`, which loses the audit-trail link to the original transaction and breaks `payment_status_transitions` chain).
2. `Database.TRANSACTIONS_GATEWAY_VALUES = frozenset({"nowpayments", "tetrapay", "admin", "gift"})` — Zarinpal was never added. `list_transactions(gateway="zarinpal")` validated the filter against this set and raised `ValueError: unknown gateway filter`. The web handler caught the ValueError and rendered an error banner, so an admin investigating "show me all the Zarinpal payments via the gateway dropdown" hit a dead end. The ledger is happily persisting `gateway='zarinpal'` rows; only the read-side filter-allowlist was missing the value.
3. `web_admin._collect_ipn_health` collected NowPayments + TetraPay drop counters but never read `zarinpal.get_zarinpal_drop_counters()`. The dashboard IPN-health tile showed 0 Zarinpal drops by omission, while in reality the per-process counters (`missing_authority`, `non_success_callback`, `unknown_invoice`, `verify_failed`, `replay`) accumulated invisibly. A wave of forged-callback traffic against `/zarinpal-callback`, or a surge of browser-close races leaving authorities pointing at unknown invoices, would have been silent in the operator-facing dashboard until someone grepped the logs by hand.

Same regression class for all three — Step-E #8 added `gateway='zarinpal'` to the *write* side (the ledger) but not the *read* side (the filter allowlist + the refund allowlist + the dashboard observability tile). Fixed by adding `"zarinpal"` to both frozensets, extending `_collect_ipn_health` with a third try-block + a `zarinpal_drops` dict + `zarinpal_total` count, and growing the dashboard template with a third "Zarinpal" sub-tile under "IPN health" (mirroring the existing TetraPay sub-tile structure exactly so a future fourth gateway can be slotted in by template-and-test diff). Test coverage: pin updated to include `"zarinpal"` in `REFUNDABLE_GATEWAYS` (`test_refundable_gateways_constant`), new `test_transactions_gateway_values_includes_zarinpal` (mirrors the existing TetraPay test in `test_tetrapay.py`), `test_dashboard_renders_ipn_health_tile_with_drop_counts` and the `_all_zero_message` companion both extended to assert the Zarinpal sub-tile renders + the empty-state message reads "no Zarinpal callback drops recorded since startup".
