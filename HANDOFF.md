...
| # | Title | Priority | Status |
| --- | --- | --- | --- |
| **Stage-12-Step-A** | **Refunds / chargebacks admin UI.** A Stage-11 user disputing a TetraPay charge currently has no in-product path to a refund — the panel can already show SUCCESS rows but can't transition them to REFUNDED. Add `/admin/transactions/{id}/refund` (POST, CSRF-protected, audit-logged) that calls a new `Database.refund_transaction(transaction_id, reason, admin_id)` method. Method semantics: SELECT … FOR UPDATE the row, refuse if `status != "SUCCESS"` or `gateway not in {"nowpayments", "tetrapay"}` (admin/gift adjustments are reversed via `admin_adjust_balance`, not the refund flow), debit the user's wallet by `amount_usd_credited` (refuse if it would drive balance negative — admin gets a friendly "user spent it; debit manually first" banner), flip status → `REFUNDED`, write `refunded_at` + `refund_reason` columns (alembic 0012), record `refund_issued` audit row. NowPayments doesn't have a programmatic refund API for confirmed crypto payments (the docs explicitly say so) — this is a **bookkeeping** refund only; the operator separately settles the user off-chain / off-card and we record that we did. TetraPay's `/api/refund` is documented; a Stage-12-Step-A.5 follow-up could call it inline, but this Step-A is gateway-agnostic. **Bug-fix candidate (to confirm during the PR):** `Database.mark_transaction_terminal` accepts `REFUNDED` (per `TERMINAL_FAILURE_STATUSES`) but does NOT debit the wallet — a future caller using `mark_transaction_terminal("REFUNDED")` instead of `refund_transaction` would silently mint money. Tighten by removing `REFUNDED` from `TERMINAL_FAILURE_STATUSES` and adding it as a separate `REFUND_STATUSES` set so the type system catches the mistake. | P0 correctness | ⏳ pending |
| **Stage-12-Step-B** | **Stuck-payment proactive admin DM.** Stage-9-Step-9 surfaces "Pending payments: N (oldest: Xh ago)" on the dashboard, but an admin who isn't actively looking at the dashboard has no way to know an invoice is stuck — IPN delivery delay, gateway flap, or webhook misconfiguration can pile up `PENDING` rows for hours before anyone notices. New background loop `pending_alert_loop(bot)` (cadence `PENDING_ALERT_INTERVAL_MIN`, default 30 min) that DMs every `ADMIN_USER_IDS` when *any* `PENDING` row's age exceeds `PENDING_ALERT_THRESHOLD_HOURS` (default 2). Per-row alert key = `(transaction_id, hour_bucket)` so the same stuck row doesn't spam the same alert every 30 min — once per hour-bucket per transaction. Mirror the Stage-10-Step-D price-alert fault isolation: `TelegramForbiddenError` per admin is logged INFO and skipped; `TelegramAPIError` is logged and skipped; the loop never crashes. Bootstrap: dedupe state lives in process memory, so a restart can re-alert once on already-stuck rows — that's intentional, an operator who deployed expects the alert to re-fire. **Bug-fix candidate:** the dashboard pending tile reads `MIN(created_at)` but the reaper threshold (24h) is the rule that decides "stuck"; we should also expose `pending_payments_over_threshold_count` on the metrics dict so the new alert and the dashboard tile stay aligned, and so an admin DM can say "3 invoices over 2h, oldest 8h" instead of just "oldest 8h". | P1 ops | ⏳ pending |
