...
| **Stage-9-Step-4** (this PR) | IPN webhook replay-dedupe. New `payment_status_transitions` append-only audit table (alembic migration `0006_payment_status_transitions`) keyed by `UNIQUE(gateway_invoice_id, payment_status)`; `payment_webhook` calls `Database.record_payment_status_transition` (`INSERT … ON CONFLICT DO NOTHING RETURNING id`) BEFORE any state mutation. A duplicate `(invoice, status)` delivery returns `None` and the handler bails with a 200 — preventing a backdated PARTIAL arriving after SUCCESS, or a duplicate `finished` retry, from re-firing `finalize_payment`. The existing row-status guards in `finalize_payment` / `finalize_partial_payment` / `mark_transaction_terminal` stay in place as defense-in-depth. New helpers: `_classify_ipn_outcome` (`applied` / `noop` / `unhandled`), `_bump_ipn_drop_counter`, `get_ipn_drop_counters`. New `Database.list_payment_status_transitions` (newest-first, optional invoice filter) so a future `/admin/payment_health` page can render forensic detail without grep-fu. **Bug fix bundled:** the missing-`payment_id` IPN drop was logged at *warning* with no counter — a misconfigured sandbox callback URL hitting the prod webhook was effectively invisible to deploy alerting. Bumped to *error* and added a process-local `_IPN_DROP_COUNTERS` dict (`bad_signature`, `bad_json`, `missing_payment_id`, `replay`) exposed via `get_ipn_drop_counters()`. Also tightened the JSON-parse path so a non-JSON body (e.g. a sandbox client posting form-encoded data) returns 200 with a clean drop reason instead of falling through to the outer `except Exception` and 500-ing back to NowPayments. The dedupe path itself fails *open* on a transient asyncpg pool blip — the row-level guards still drop replays, and we'd rather process the IPN than 500 and trigger a NowPayments retry storm. 16 new tests (523 total): 4 dedupe / row-guard interplay tests, 4 outcome-classification tests, 4 bug-fix-bundle drop-counter / log-level pins, 4 `Database.record_payment_status_transition` / `list_payment_status_transitions` SQL-shape tests. | P1 correctness | ✅ this PR |
