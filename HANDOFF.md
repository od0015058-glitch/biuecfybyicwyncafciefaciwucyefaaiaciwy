...
| 10 | **Image / vision model support** — let users send photos and have vision models (GPT-4V, Claude 3) analyze them. OpenRouter supports multimodal; need to handle Telegram photo downloads + base64 encoding in `ai_engine`. **STARTED — see "Stage-15-Step-E #10" section below for the `vision.py` foundation slice (pure helpers: `is_vision_capable_model`, `encode_image_data_uri`, `build_multimodal_user_message`) + bundled persistence-after-charge double-billing fix in `ai_engine.chat_with_model`.** | P2 product | Medium | Popular user request for AI bots |
...
* **Configurable window selector** — the page currently hardcodes a 30-day window via `_MONETIZATION_DEFAULT_WINDOW_DAYS`. A natural extension is a `?window=7|30|90` query-param dropdown (the underlying `Database.get_monetization_summary(window_days=...)` already accepts arbitrary values).
...
What remains for future Step-E #10 PRs (the user-visible feature lands when these all ship together, or in a tight stack):

* **Photo-message handler** — new aiogram `@router.message(F.photo)` handler in `handlers.py`. Picks the largest `PhotoSize` from `message.photo[-1]`, calls `bot.download(file_id)` to a `BytesIO`, runs through `vision.encode_image_data_uri(content_type="image/jpeg")` (Telegram's photo-message channel always re-encodes to JPEG so the content type is fixed), and stashes the data URI on FSM state alongside any caption. If `caption` is present (user typed a question with the photo), enqueue a synthetic chat-with-model call carrying both the prompt and the data URI; otherwise prompt the user "what would you like to ask about this image?" and await the next text message before enqueuing.
* **`ai_engine.chat_with_model` multimodal path** — accept an optional `image_data_uris: list[str]` parameter. When non-empty AND `vision.is_vision_capable_model(active_model)` → assemble the payload via `vision.build_multimodal_user_message`. When non-empty AND the model is NOT vision-capable → return a localised "this model can't see images, switch to a vision model" message via a new `t(lang, "ai_model_no_vision")` slug. When empty → existing text-only path, unchanged.
* **Memory persistence for image turns** — the current `conversation_messages` table stores plaintext `content`. A vision turn's "content" is the multimodal content array, not a string. Either: (a) extend the schema to a JSONB column and update `get_recent_messages` to round-trip the array shape, or (b) only persist the text portion of the multimodal turn (lossy but cheap) — caption-only on the user side, full reply on the assistant side. Option (b) is recommended for the first integration PR because it keeps the schema migration out of the critical path; option (a) follows in a later slice.
* **Token / cost accounting for image turns** — vision images consume input tokens (a 1024×1024 image is ~765 tokens for GPT-4V; varies per model). The existing `pricing.calculate_cost_async` only takes `(active_model, prompt_tokens, completion_tokens)` — those values come back from OpenRouter's `usage` block, which already includes the image-token contribution, so this *should* be transparent. But: a user fires an oversize image, gets the local 400 from `encode_image_data_uri`, and pays nothing — verify this is the right behaviour (it is — we never called OpenRouter so there's nothing to charge for). Document explicitly in a code comment so a future PR doesn't add a "charge for the rejection" foot-gun.
* **HEIC / unsupported-mime conversion path** — iPhone users send HEIC by default. The current MIME allowlist rejects it. Either: (a) reject loudly with a localised "please re-send as JPEG/PNG" message, or (b) install Pillow + the HEIC plugin and convert server-side. Option (a) is the trivial path; option (b) doubles the install footprint and adds a CPU-hungry hot path. Defer until we see real user demand.
* **Per-image cost transparency** — the wallet UI / charge log doesn't currently surface "this $X charge included a vision image". Could be useful for a power user trying to budget. Out of scope for the first integration PR.
