...
| #6.20 | **`create_promo_code` / `create_gift_code` reject non-finite amounts** (this PR). Defense-in-depth gap parallel to #6.19. Both DB-layer create methods already reject negative / zero / over-DECIMAL-cap amounts, but a NaN slipped through every guard: `NaN <= 0` is `False` in Python, `NaN > 999_999.9999` is `False`, and PostgreSQL `NUMERIC` happily stores `'NaN'::numeric` (it's a valid IEEE-754 value PG accepts). Once landed in `promo_codes.discount_amount` or `gift_codes.amount_usd`, the NaN propagates: a fixed-amount promo redemption runs `compute_promo_bonus` → multiplies by NaN → `transactions.promo_bonus_usd = NaN` → `finalize_payment` runs `balance_usd + NaN` and bricks the wallet exactly the way PR #75 prevented at the IPN layer; a gift-code redemption runs `balance_usd + NaN` directly via `redeem_gift_code`. The web admin form parsers (`parse_promo_form`, `parse_gift_form`) and the Telegram-side parser (`parse_promo_create_args`) all reject NaN / ±Infinity up-front today, so the only present route to the hole is a hypothetical caller bypassing the parsers (a future JSON API, a refactor that drops the parser, a test stub) — but that's exactly the same threat model `_is_finite_amount` was introduced for in PR #75 / #77 and now guards `deduct_balance` / `finalize_payment` / `finalize_partial_payment` / `admin_adjust_balance`. Fix: hoist `_is_finite_amount` checks to the very top of the validation chain in both `create_promo_code` (before `<= 0` / `> 999_999.9999`) and `create_gift_code` (before `<= 0` / `> GIFT_AMOUNT_MAX`). Refusal raises `ValueError("… must be a finite number (NaN / ±Infinity rejected)")` with the same shape as the existing money-path guards, so log lines surface the bad caller. 6 new tests in `tests/test_web_admin.py` covering NaN / +Inf / -Inf for both creators (existing 802 → 808 total on this branch; once PR #85's 3 tests merge the cumulative count is 811). |
