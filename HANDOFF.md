...
| **Stage-9-Step-6** (this PR) | Soft-cancel for running/queued broadcasts. New `POST /admin/broadcast/{id}/cancel` endpoint flips `job["cancel_requested"] = True` in the in-memory registry; `admin._do_broadcast` polls the flag at the top of every send-loop iteration via the new `should_cancel` kwarg and exits cleanly with `cancelled=True` in the stats dict. `_run_broadcast_job` then transitions the job to a new `state="cancelled"` (instead of `completed`) and writes the actual attempted-recipient count to `job["i"]` so the live-progress page renders "stopped at N/total" correctly. The cancel is **cooperative** — `task.cancel()` is intentionally NOT used because a `CancelledError` mid-send would be miscounted as a recipient failure. The detail-page template grew a Cancel button (CSRF-protected, hidden once the job hits a terminal state, disabled while cancellation is in-flight) and a yellow `state-cancelled` pill. The eviction allow-list in `_store_broadcast_job` now treats `cancelled` jobs as eligible for eviction so they don't pile up forever. **Bug fix bundled:** `admin._do_broadcast` previously called `await asyncio.sleep(exc.retry_after)` with NO upper bound on `exc.retry_after`. A misbehaving Telegram response (or a real outage spike returning `retry_after=3600`) would pin the broadcast worker for an hour per affected recipient with no log explaining why the operator was watching an apparently-stuck job. Capped at `_BROADCAST_RETRY_AFTER_MAX_S = 60s`; if the server-supplied window exceeds the cap we log a `WARNING` with both raw and capped values so prolonged degradation surfaces in ops dashboards. (HANDOFF's originally-spec'd bug — the eviction cap allowing a `running` job to be pushed off the end — was already fixed in Stage-8-Part-5 PR #59 by guarding eviction on `state in {completed, failed}`. The retry-after cap is a separate, real, latent DoS surface I found while wiring soft-cancel.) 15 new tests (522 total): 4 `_do_broadcast` cancel pins (default `cancelled=False`, flip-after-first short-circuits, predicate exception swallowed, top-of-loop check), 2 `_do_broadcast` retry-after pins (capped at 60s, normal value unchanged), 1 `_run_broadcast_job` end-to-end pin (`state="cancelled"` + `i = sent+blocked+failed`), 1 eviction pin (`cancelled` jobs evictable), 6 `/admin/broadcast/{id}/cancel` endpoint pins (auth, CSRF, unknown job, running-job sets-flag-and-audits, terminal-job-rejected, idempotent), 1 template render pin (cancel button shown for running, hidden for completed). | P1 correctness | ✅ this PR |
