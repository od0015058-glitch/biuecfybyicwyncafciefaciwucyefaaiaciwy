...
## 10c. Stage-16 — Future roadmap (product & ops gaps, added 2026-05-03)

The §10b roadmap closed at 30/30 with PR #192 (per-gateway model
disable). User then asked for a holistic audit of *the bot itself*
(not just the admin panel) and a second AI flagged four ops gaps
on top. This section is the merged 34-item roadmap. Same conventions
as §10b — pick the next pending row, ship one PR, mark `Shipped — PR #N`,
move to the next.

Priority tags follow the project framework (P0 security > P1
correctness/money > P2 product > P3 ops). Where the user's wording
disagreed with the framework, I kept the user's wording and noted
the framework reading inline so the next AI can re-prioritise if
they need to.

| #  | Gap | Today | Target | Priority | "What it'd take" | Status |
|----|-----|-------|--------|----------|------------------|--------|
| 1  | **Voice messages** — user sends a Telegram voice note. | `F.voice` / `F.audio` are not registered; bot silently ignores the upload. | New handler that downloads the OGG, runs Whisper (OpenRouter has an audio model surface) or OpenAI's `whisper-1`, then feeds the transcribed text into the existing `chat_with_model` pipeline as a regular user turn. Charge a per-second STT add-on on top of the chat charge. | P2 product | New `voice.py` module mirroring `vision.py` (download → encode → call STT → return text). Wire `@router.message(F.voice)` and `@router.message(F.audio)`. The current per-user inflight cap and rate-limit middleware already apply. ~2 days. | Pending |
| 2  | **Image generation** — `/imagine <prompt>` produces an image reply. | No image-out path. Bot has vision-IN (row 30 of §10) but no vision-OUT. | New `/imagine` command that calls an OpenRouter image model (`stability-ai/stable-diffusion-3` or `openai/dall-e-3`), charges the wallet at the per-image rate, and sends the result as a Telegram photo. | P2 product | New module `image_gen.py`. New handler in `handlers.py` for `Command("imagine")`. Pricing row in `models_catalog.py`. Per-user budget pre-check (image gen is way more expensive than chat tokens). ~2-3 days. | Pending |
| 3  | **Text-to-speech replies** — bot replies with a voice note in addition to (or instead of) text. | Text-out only. | Per-user "voice replies on/off" toggle in the wallet menu. When on, after each chat reply, call ElevenLabs / OpenAI-TTS / OpenRouter-TTS and send the audio as a `sendVoice`. Persian-speaking users learning English are a big use case here. | P2 product | New module `tts.py`. New user pref column `tts_enabled BOOLEAN`. Inline-keyboard callback for the toggle. Charge per character. ~2 days. | Pending |
| 4  | **Streaming responses** — token-by-token incremental UI. | Bot waits for the full LLM response, then sends one Telegram message. Long answers feel slow. | Refactor `ai_engine.chat_with_model` to use OpenRouter's `stream=True`. Send a placeholder Telegram message, edit it via `editMessageText` every ~1s with the accumulated tokens. Stop on the final `[DONE]` chunk. | P2 product | Telegram caps `editMessageText` at 30 calls/sec per chat — debounce to 1 edit/sec to stay safe. The existing `chunk_for_telegram` chunker already handles the "message too long" case for the final state. Watch out for markdown rendering during a streaming edit (we re-render `_safe_md` each tick). ~1 day — biggest UX win for the time. | Pending |
| 5  | **Web search / current events** — LLMs have a training cutoff. | No live-web augmentation. | New `/search` command OR tool-calling integration: SerpAPI / Tavily / Brave Search returns top-N URLs + snippets, injected into the prompt as system-context. Distinct usage_log row tagged `web_search` so we can charge separately if we want. | P2 product | Pick a single provider (Tavily is cheapest at scale, ~$0.005/search). Add `WEB_SEARCH_API_KEY` env var + module `web_search.py`. Hot-path injection in `chat_with_model` when prompt matches a "current-events" heuristic OR when the user prefixes `/search`. ~1-2 days. | Pending |
| 6  | **PDF / document Q&A** — user sends a PDF, bot answers questions over it. | `F.document` only catches *image* documents (HEIC etc.) and refuses everything else. | Detect `mime_type == "application/pdf"` (and a few others — `.docx`, `.txt`, `.md`). Extract text via PyMuPDF (PDF) / python-docx (DOCX) / plain decode. Stuff into the prompt with a "Document context: …" preamble. Charge per extracted-token. | P2 product | Hard cap on extracted size (e.g. 100 K chars) to protect the wallet. New ext-input column on `usage_logs` for "input came from document". ~3 days incl. proper extraction error handling. | Pending |
| 7  | **Tool / function calling** — let the model invoke calculator, web search, code execution. | Pure text completion; no `tool_calls` plumbing. | Wire OpenRouter's `tools` param. Implement N tool handlers server-side (calculator, web search, currency convert, weather). `chat_with_model` loops on tool calls until the model emits text. | P2 product | Significant — touches the chat loop's central dispatch. Most providers (OpenAI, Anthropic, Gemini) speak the same `tools` schema via OpenRouter so it's one shape. ~1 week. | Pending |
| 8  | **Inline mode** — `@meowassist_bot question` in any chat. | Not registered. | `BotFather → /setinline → enable`, plus `@router.inline_query()` handler that runs the prompt through OpenRouter and returns `InlineQueryResultArticle`. Crucial acquisition channel — every share is a free demo to the receiver's chat. | P2 product | The wallet model needs a special-case: charge the inviting user, NOT the receiver. Most projects skip inline charging and treat it as paid marketing. ~1-2 days. | Pending |
| 9  | **Group-chat support** — bot can be added to groups. | DM-only by middleware. | Group billing model: "group owner pays for the whole group" (DB column on `bot_chats`). Or "use the @-mentioning user's wallet". OR refuse and force admin-pre-approval. | P2 product | Big design call before any code — pick the billing model first. After that, ~1 week of work to gate every existing handler. | Pending |
| 10 | **Conversation forking / multiple threads** — user has separate "work" / "personal" / "study" contexts. | Single linear conversation per user. | New `conversation_threads` table with `user_id`, `name`, `is_active`. UI: `/threads` command lists them, `/new_thread <name>` creates one, tap to switch. The active thread feeds memory. | P2 product | DB migration + new admin column for thread quota. ~3 days. | Pending |
| 11 | **User-customisable system prompts / personas** — "always reply in formal Persian", "act as a Python tutor". | None. | Per-user `system_prompt TEXT` column on users + UI to set it. Premium feature on subscription tiers (gap #15). | P2 product | Trivial DB-wise (one nullable column, plumb into `chat_with_model`'s message-construction). The hard part is the UX (multi-line editor on Telegram is awkward — probably best done via the web admin panel as a "settings" page). ~2 days. | Pending |
| 12 | **Regenerate / Stop / Edit-last-message buttons** — under each AI reply. | If reply is bad, user must retype. | Inline-keyboard with three buttons: 🔄 Regenerate (re-sends the same prompt with `temperature += 0.1`), ⏹ Stop (abort an in-flight stream — depends on #4), ✏️ Edit (FSM state to capture a replacement prompt). | P2 product | Regenerate is independently shippable (~half a day). Stop only matters once #4 streaming is in. Edit is FSM-fiddly but ~1 day. | Pending |
| 13 | **Telegram Stars (XTR)** — Telegram's native in-app currency. | Not integrated. | Aiogram supports `sendInvoice(currency="XTR")` natively. New gateway entry `telegram_stars` in `payments.py`, IPN equivalent comes via aiogram's `pre_checkout_query` → `successful_payment` → wallet credit. Faster checkout than crypto, no card needed. | P1 money | One new module `telegram_stars.py`. New row in `_GATEWAY_PROVIDER_LIST` (admin already has master switch + per-key disable from row 14 of §10b). FX rate: Telegram pays out 0.01 USD/Star roughly — pin a `STAR_TO_USD_RATE` env. ~2-3 days. | Pending |
| 14 | **Fiat card top-up (Stripe / PayPal / Lemon Squeezy)** — non-IRR, non-crypto. | Crypto via NowPayments + IRR cards via TetraPay/Zarinpal. No USD/EUR card. | New gateway module per provider. Stripe is the easiest (well-documented webhook, PaymentIntent flow). | P1 money | Pick one provider first. Stripe webhook handler in `web_admin.py` (or its own file). New gateway key `stripe`. ~3-5 days for the first one, half a day each for additional providers. | Pending |
| 15 | **Subscription tiers** — recurring monthly plans. | Pay-as-you-go wallet only. | New `subscription_plans` table (price, name, monthly credit, model allowlist). New `user_subscriptions` table (user, plan_id, period_start, period_end, status). Admin panel page to manage plans. Per-message check: subscription budget first, then wallet. | P2 product | Touches the spend hot-path. Significant tests required. ~1-2 weeks. | Pending |
| 16 | **Team / org accounts** — small business gives N seats one shared balance. | Per-user only. | New `organizations` + `org_members` tables. Wallet column flips to `wallet_owner_id` (user OR org). Per-seat usage caps + monthly auto-reset. Org admin invites via signed link. | P2 product | Major architectural change touching every wallet-affecting code path. ~2-3 weeks. | Pending |
| 17 | **Real `/health` endpoint** — actual probe of every dependency. | `GET /telegram-webhook/healthz` returns `{"status":"ok"}` only when webhook mode is on. Web-server-is-alive check, not bot-is-healthy. | New `GET /health` (or `/healthz` on root, NOT under telegram-webhook). Probes: `SELECT 1` on Postgres, Redis `PING`, lightweight OpenRouter `models` call with short timeout, current status of every registered background loop (reaper, fx_refresh, alerts, discovery). Returns JSON with per-component status + HTTP 200 / 503. So an external uptime monitor (UptimeRobot, BetterStack) can hit this every 60s and page on real outages. | P1 ops | **Shipped — PR #194** (new `health.py` module exposing `GET /health` and `GET /healthz` aliases on the same aiohttp app as `/admin/*` and `/metrics`. Probes Postgres `SELECT 1`, Redis `PING`, OpenRouter `/models`, every registered background loop (`bot_health.LOOP_CADENCES`). Per-probe 3s timeout (`HEALTH_PROBE_TIMEOUT_SECONDS`), 5s in-process snapshot cache (`HEALTH_CACHE_SECONDS`) with `X-Health-Cache: hit/miss` debug header. HTTP 200 for ok/degraded, 503 only when Postgres is down so a TCP-level monitor still pages. Public-safe body (no secrets, no internal hostnames). Mounted unconditionally — fixes the long-poll-mode gap where `/telegram-webhook/healthz` was scoped to webhook mode only. 35 new tests. Total suite: 3824 passing.) |
| 18 | **Grafana dashboard + Prometheus AlertManager** — visual ops + automated paging. | `/metrics` exists with all the counters (IPN drops, loop heartbeats, `bot_status_score`, etc.) but no dashboard, no AlertManager rules. Operator has to manually check `/admin/control`. | New `docker-compose.grafana.yml` override that spins up Grafana + Prometheus + AlertManager. Pre-built dashboard JSON for revenue, error rates, model latency, pending payments, wallet health. AlertManager rules: `bot_status_score >= 4`, pending payments stuck >1h, OpenRouter error rate >5%, container memory near limit. | P1 ops | New compose file + `dashboards/meowassist.json` + `alerts/meowassist.rules.yml`. Provision Grafana with the bot's `/metrics` Prometheus endpoint as a scrape target. Alert receiver: Telegram bot DM (use a separate ops bot, not the user-facing one) OR Slack webhook OR PagerDuty. ~2-3 days. | Pending |
| 19 | **AI response feedback (👍 / 👎 rating)** — under every AI reply. | None. | Two inline-keyboard buttons under each AI reply. Tap records the rating alongside the `usage_logs` row (new `feedback` column, nullable TEXT, values `positive` / `negative`). If rolling 1h dissatisfied rate crosses a configurable threshold (e.g. >30% negative), DM admins with `⚠️ High dissatisfaction rate` alert naming the model(s) involved. Lets us proactively disable a misbehaving model before users churn. | P2 product | New nullable column on `usage_logs` (alembic). Callback handler `feedback_pos_<usage_id>` / `feedback_neg_<usage_id>`. New background task in `bot_health_alert.py` that windows the rate. ~2 days. | **Shipped — PR pending merge** (this PR — alembic 0020 adds nullable `feedback TEXT` column to `usage_logs` with named CHECK constraint (`positive`/`negative`/`NULL`) and partial covering index on `(model_used, created_at DESC) WHERE feedback IS NOT NULL` so the dissatisfaction-rate aggregate range-scans only rated rows; `database.log_usage` switched from `execute` to `fetchval` to return the inserted SERIAL `log_id`; new `database.record_usage_feedback(log_id, telegram_id, feedback)` enforces the per-user owner check (you can't rate someone else's reply) AND first-tap-wins idempotency (`feedback IS NULL` guard) in a single UPDATE; new `database.get_recent_feedback_rates(window_seconds, min_samples)` returns the per-model dissatisfaction snapshot; new `ai_feedback.py` module with `build_feedback_keyboard(log_id)` (👍 `fbp:<log_id>` / 👎 `fbn:<log_id>` callback_data, 23 bytes max — well under Telegram's 64-byte cap), `parse_feedback_callback`, `run_dissatisfaction_check` (per-model trailing-window negative-rate aggregate, latched alerting, per-admin fault-isolated DM), and `dissatisfaction_alert_loop` registered via `@register_loop("ai_feedback_alert", ...)` so it heartbeats `meowassist_ai_feedback_alert_last_run_epoch` like every other loop; `handlers.process_chat` and `handlers.process_photo` thread an `out_meta` dict through `chat_with_model` to capture the just-inserted `log_id` and attach the keyboard to the LAST chunk of the reply only; new `process_feedback_callback` decodes the tap, calls `record_usage_feedback`, surfaces a localised toast (en + fa), and strips the keyboard via `edit_reply_markup(reply_markup=None)`; eight env knobs: `AI_FEEDBACK_ENABLED` (master, default true), `AI_FEEDBACK_ALERT_ENABLED` (alert layer, default true), `AI_FEEDBACK_DISSATISFACTION_THRESHOLD_RATIO` (default 0.30), `AI_FEEDBACK_DISSATISFACTION_WINDOW_SECONDS` (default 3600), `AI_FEEDBACK_MIN_SAMPLES` (default 10), `AI_FEEDBACK_LOOP_INTERVAL_SECONDS` (default 300, floor 30), `AI_FEEDBACK_ALERT_COOLDOWN_SECONDS` (default 3600). Bundled bug fix: `abuse_detection._REPETITION_PATTERN` now compiles with `re.DOTALL` so a payload of 200+ `\n` characters in a row classifies as `spam_repetition` instead of slipping past every layer (pre-fix `.` did not match newlines, so a pure newline-flood was forwarded to OpenRouter). 64 new tests. Total suite: 3920 passing.) |
| 20 | **Abuse / spam detection** — content-based + spend-spike. | Per-user rate limit (token bucket) + per-message in-flight caps. No content-based detection. No spend-spike alerting. | Lightweight middleware (or check inside `process_chat`) that: (a) rejects prompts longer than configurable max (e.g. 50K chars), (b) regex-scans for common injection / payload patterns (SQL injection, HTML/JS injection, repeated-character flood), (c) if a single user burns >$5 in <10 min, auto-DMs admins with `⚠️ Possible abuse: user X spent $Y in Z minutes`. Alert-first, block-only-on-clear-signal. | P0 security (user wrote P1 security; security beats correctness in framework) | New module `abuse_detect.py`. Three configurable env knobs (`ABUSE_MAX_PROMPT_CHARS`, `ABUSE_SPEND_SPIKE_USD`, `ABUSE_SPEND_SPIKE_MINUTES`). Plumb into the chat hot-path. ~1-2 days. | **Shipped — PR #195** (new `abuse_detection.py` module with three independent layers: (1) `is_oversized` length cap via `ABUSE_MAX_PROMPT_CHARS` (default 50 000, floor 1 000 so an env-typo can't silently disable), (2) `classify` regex scan for SQL-injection (`UNION SELECT`, `DROP/TRUNCATE/ALTER TABLE`, `OR 1=1`, `; --`), XSS (`<script`, `javascript:`, `onerror=`), shell-injection (`$(curl`, `wget http`) and 200+-char repetition spam, (3) `SpendSpikeTracker` sliding-window per-user spend tracker with idempotent latch (window: `ABUSE_SPEND_SPIKE_WINDOW_SECONDS` default 600, threshold: `ABUSE_SPEND_SPIKE_THRESHOLD_USD` default 5.0). Each layer toggle-able via `ABUSE_DETECTION_ENABLED` (master), `ABUSE_PATTERNS_ENABLED`, `ABUSE_SPEND_SPIKE_ENABLED`. The classifier short-circuits in `process_chat` BEFORE `consume_chat_token` (an oversized / probe-shaped prompt doesn't burn a rate-limit slot a legitimate user could otherwise use). Same `ai_local_rate_limited` reply for every non-OK classification so an attacker can't probe which rule fired. Spend-spike: `ai_engine.chat_with_model` calls `record_paid_spend` after settlement; the chat handler drains the latch via `maybe_alert_spend_spike(message.bot, user_id=...)` after the AI reply is sent so an admin DM 5xx can't lose the user's reply. Per-admin fault isolation matches `model_discovery.notify_admins`. **Bundled bug fix**: pre-PR `process_photo` flowed `message.caption` straight through to `chat_with_model` unchecked — a 1 MB caption paired with a single photo upload would burn one rate-limit slot but cost millions of OpenRouter prompt tokens. The new abuse pre-check runs on the photo caption too, closing the hole at zero extra cost. 32 new tests (3790 → 3822). Env vars `ABUSE_DETECTION_ENABLED`, `ABUSE_PATTERNS_ENABLED`, `ABUSE_SPEND_SPIKE_ENABLED`, `ABUSE_MAX_PROMPT_CHARS`, `ABUSE_SPEND_SPIKE_WINDOW_SECONDS`, `ABUSE_SPEND_SPIKE_THRESHOLD_USD` documented in `.env.example`.) |
| 21 | **Automated DB backups** — to off-server storage. | Manual `pg_dump` documented in setup guide; no cron. | Cron snippet that runs nightly, gzips the dump to `/var/backups/meowassist/`, keeps last 14 days, optionally rsyncs / `aws s3 cp` to off-server. **Critical: a single `rm` of the postgres volume right now wipes every transaction record.** | P1 ops | One `meowassist-backup.sh` shell script + one `/etc/crontab` line. Document in `MEOWASSIST_SETUP_2026.md` (already done in the latest setup-guide attachment). Optional: add an admin-panel page to download the latest backup tarball. ~half a day for the script + cron, +1 day for the admin-panel page. | Pending |
| 22 | **Real-time chat watching** — admin tails a live conversation. | Admin can export a user's conversation as `.txt` (row 16 §10b shipped this session); can't tail. | New `/admin/users/<id>/live` page that subscribes to the user's incoming/outgoing messages via Server-Sent Events. Useful for support cases ("show me what they're typing right now"). Audit-logged per session. | P3 ops | aiohttp SSE handler + Redis pub/sub channel that the chat handler publishes to. ~2-3 days. Privacy + audit care needed. | Pending |
| 23 | **Email / Slack / Discord alerts** — out-of-band when the bot itself is down. | Bot-health alerts go out as Telegram DMs only. If the bot is down, the DMs are also down. | Pluggable `alert_sinks.py` with three implementations: SMTP, Slack webhook, Discord webhook. Existing `bot_health_alert.py` fan-out picks every configured sink. | P1 ops | New module + 3 env vars (`ALERT_SMTP_*`, `ALERT_SLACK_WEBHOOK`, `ALERT_DISCORD_WEBHOOK`). ~1-2 days. | Pending |
| 24 | **Sentry / error tracking** — exception aggregation + regression tracking. | Exceptions land in Docker logs. No `sentry_sdk` integration. | Add `sentry-sdk[aiohttp,asyncpg]` to requirements. Init in `main.py` if `SENTRY_DSN` env is set. Tag events with `release` (git SHA) for regression hunting. | P1 ops | One PR adds the dependency + init. ~3 hours. | Pending |
| 25 | **CSV import for promo / gift bulk creation** — operator imports a list. | Operator creates promo codes one at a time on `/admin/promos`. | New POST `/admin/promos/import` accepting a CSV with columns matching the create form. Server-side validates each row, reports per-row success/failure summary. | P3 ops | New web route + CSV parser (`csv` stdlib is fine — same defang sentinels as the export hub from row 27 §10b). ~1 day. | Pending |
| 26 | **Staging environment** — second identical deploy. | Single-machine, no documented staging. | New `MEOWASSIST_STAGING.md` + a separate compose file template that deploys to `staging.meowassist.space` with `BOT_TOKEN_STAGING` (a separate bot from BotFather pointing at the same code). | P3 ops | Pure documentation + a thin compose override. ~half a day. | Pending |
| 27 | **GDPR / "delete all my data" self-service** — `/forget_me` command. | User can clear conversation history but can't wipe wallet, transactions, referral code. | New `/forget_me` command with a 24-hour cooling-off window (DM confirmation). Hard-deletes user row + cascade. Audit-logged for compliance. **Care:** consider a "tombstone" row to satisfy financial-record retention requirements (in some jurisdictions, refund records must be kept 5+ years). Talk to a lawyer. | P0 security (compliance) | DB schema changes + handler + audit slug `user_self_deletion`. ~2-3 days incl. the legal-review pass. | Pending |
| 28 | **Per-user data export at user request** — `/export_my_data` JSON download. | Admin can export anyone; user has no self-service. | New `/export_my_data` command that streams a JSON of every record about the user (wallet history, transactions, conversations, audit, referrals). Same defang as the existing CSV export hub. | P0 security (compliance) | Re-uses the existing `iter_*` generators from row 27 §10b. ~2 days. | Pending |
| 29 | **Registration CAPTCHA / bot-abuse mitigation** — gated free trial. | Anyone with a Telegram account can `/start` and consume the free messages quota. | Either: (a) integrate Telegram's native `passport` field for verified phone numbers, OR (b) require the user to complete a math puzzle / channel-subscription proof before the free quota kicks in. Optional override for trusted referrers. | P0 security | Layer in front of `Database.create_user`. ~2-3 days. | Pending |
| 30 | **`/clear` shortcut on wallet menu** — quick-access "new chat". | Users have to scroll up and tap the "🆕 New Chat" hub button. | New row on the wallet inline-keyboard. Trivial. | P3 product polish | Half a day. | Pending |
| 31 | **`/undo` to remove just last user→bot turn** — granular vs. all-or-nothing clear. | Clear-history is all-or-nothing. | New `/undo` command that pops the last two rows from `messages` (user + bot). | P3 product polish | Half a day. | Pending |
| 32 | **"What model am I on right now?" indicator** — surfaced in the chat reply. | Active model name shown only in the menu. | Append a small footer to each AI reply: `_via openai/gpt-4o_` or similar. Per-user toggle to suppress. | P3 product polish | ~1 hour. | Pending |
| 33 | **Favourite-models pinning** — power users pin their top 3. | Models list is alphabetical / pricing order. | New `favourite_models JSONB` column on users. Render a "⭐ Pinned" section at the top of the model picker. | P3 product polish | ~1 day. | Pending |
| 34 | **Conversation export in PDF format** — alongside the multi-part `.txt` from §10b row 16. | `.txt` only. | New `/admin/users/<id>/conversations.pdf?part=N` endpoint. ReportLab or WeasyPrint to render. Includes inline images from the row 8 `image_data_uris` column. | P3 ops | ~2-3 days incl. correct unicode handling for Persian. | Pending |

### Suggested execution order (this PR's ranked-ROI from PROJECT_GAPS.md, lightly re-ordered to honour the framework P0 → P3):

1. ~~Row 17 — real `/health` endpoint (P1 ops, fast, foundation).~~ Shipped (PR #194).
2. ~~Row 20 — abuse / spam detection (P0 security).~~ Shipped (PR #195).
3. ~~Row 19 — AI feedback 👍 / 👎 (P2 product, fast).~~ Shipped (this PR).
4. **Row 4 — streaming responses (P2 product, biggest UX win). ← NEXT**
5. Row 21 — automated DB backups (P1 ops, fast).
6. Row 18 — Grafana + AlertManager (P1 ops, larger; do after 17 is in so the dashboard has `/health` to graph).
7. Row 13 — Telegram Stars (P1 money).
8. Row 24 — Sentry (P1 ops, ~3 hours).
9. Row 27 / 28 — GDPR self-service (P0 compliance, batch together).
10. Row 6 — PDF/document Q&A (P2 product).

Everything else is pickable in any order based on user demand signal.
