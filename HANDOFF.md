...
| # | Title | Priority | Status |
| --- | --- | --- | --- |
| **Stage-10-Step-A** | **Per-currency NowPayments minimum + $2 global floor + pre-flight refusal with alternative-coin suggestion.** Lower the existing $5 hard floor to $2 (`payments.GLOBAL_MIN_TOPUP_USD`, env-overridable via `MIN_TOPUP_USD`). Before we POST `/v1/payment`, proactively look up the per-currency minimum via `/v1/min-amount` (the existing `get_min_amount_usd` cache) and refuse below `max(GLOBAL_MIN_TOPUP_USD, per_currency_min)` with a message that names the min AND suggests the cheapest alternative currency that WOULD cover the user's requested amount (`find_cheaper_alternative`). New background task `refresh_min_amounts_loop(tickers, interval=15min)` warms the cache on boot and refreshes so the pre-flight check is cache-hit-fast. 3 new string keys (fa/en): `charge_min_amount_with_min_and_alt` alongside the existing `_with_min` / `_unknown` fallbacks. **Bundled UX fix:** without pre-flight, every sub-min invoice cost one wasted `POST /v1/payment` plus a generic "too small" rejection with no actionable recovery path beyond "try another coin". Pre-flight saves the round-trip AND names a concrete next step. | P1 correctness | ⏳ this PR |
| **Stage-10-Step-B** | **Transparent markup-inclusive pricing in model picker.** User feedback 2026-04-29: "right now in selecting models of chat it shows the price of same as the site. but we want som profits dont we." Today the picker renders raw OpenRouter per-1M-token prices; actual cost-per-message is `raw * PRICING_MARKUP` (see `pricing._apply_markup`). Displayed prices must match what actually gets charged. Plan: find every template / string that formats per-model prices, pipe them through the same markup math, add a `(includes service fee)` footnote so nothing is hidden. **Bundled bug fix TBD** once the markup code is re-read — I'll surface any real latent issue rather than invent one. | P2 product | ⏳ pending |
| **Stage-10-Step-C** | **Auto-discover new OpenRouter models + admin notification.** New background task (shared scaffold with Stage-10-Step-A's refresher) hits `GET https://openrouter.ai/api/v1/models`, picks the top-5 by a popularity signal per company (`openai`, `anthropic`, `google`, `deepseek`, `x-ai`), and diffs against `models_catalog`. Any new model surfaces in the web admin panel as an "Enable" toggle and sends a Telegram DM to every `ADMIN_USER_IDS` with a summary. Admin approval gates the catalog insert so we never auto-enable a model whose pricing / safety posture we haven't reviewed. Paired with Step-D so the new rows arrive with already-marked-up prices. **Bundled bug fix TBD.** | P2 product | ⏳ pending |
| **Stage-10-Step-D** | **Auto-refresh catalog prices from OpenRouter.** Same background task as Step-C also compares stored vs. live prices per model. On any delta, update the catalog row and log. On a >20% delta, additionally notify admins via the same DM channel as Step-C so a sudden upstream price spike doesn't silently eat our margin. Admin panel gains a "last price refresh" timestamp + manual refresh button. **Bundled bug fix TBD.** | P3 operational | ⏳ pending |
