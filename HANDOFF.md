...
| 20 | **Abuse / spam detection** — content-based + spend-spike. | Per-user rate limit (token bucket) + per-message in-flight caps. No content-based detection. No spend-spike alerting. | Lightweight middleware (or check inside `process_chat`) that: (a) rejects prompts longer than configurable max (e.g. 50K chars), (b) regex-scans for common injection / payload patterns (SQL injection, HTML/JS injection, repeated-character flood), (c) if a single user burns >$5 in <10 min, auto-DMs admins with `⚠️ Possible abuse: user X spent $Y in Z minutes`. Alert-first, block-only-on-clear-signal. | P0 security (user wrote P1 security; security beats correctness in framework) | New module `abuse_detect.py`. Three configurable env knobs (`ABUSE_MAX_PROMPT_CHARS`, `ABUSE_SPEND_SPIKE_USD`, `ABUSE_SPEND_SPIKE_MINUTES`). Plumb into the chat hot-path. ~1-2 days. | Pending |
