...
| # | Title | Priority | Status |
| --- | --- | --- | --- |
| **Stage-11-Step-A** | **Live USD→Toman exchange rate ticker.** Shipped: `fx_rates.py` + `alembic/versions/0010_fx_rates_snapshot.py` + `database.db.get_fx_snapshot` / `upsert_fx_snapshot`. Background `refresh_usd_to_toman_loop` (default 10-min cadence, env `FX_REFRESH_INTERVAL_SECONDS`) pulls from a configurable source (default `nobitex.ir` USDT/IRR — free, no auth; normalised to tomans by ÷10). In-memory `FxRateSnapshot` cache + single-row DB snapshot so a process restart starts with the last known-good rate rather than a cold cache (~10 min of no-rate would otherwise break the Stage-11-Step-D wallet UI and Stage-11-Step-B Toman top-up). Cache-preservation pattern matches `payments.refresh_min_amounts_once`: on fetch failure keep the prior cached value rather than collapsing (a silent collapse would either crash the wallet UI or render $0 balances). Plausibility band `[10 000, 1 000 000]` tomans per USD guards against upstream returning 0/NaN/absurd values. Admin DM via `bot.send_message` on moves ≥ `FX_RATE_ALERT_THRESHOLD_PERCENT` (default 10%, env-overridable) with per-admin fault isolation. Source is pluggable (`FX_RATE_SOURCE=nobitex|bonbast|custom_json_path|custom_static`). `main.py` spawns + cancels the refresher alongside the other background tasks. 34 new tests in `tests/test_fx_rates.py` (953 total, was 919). **Bundled improvement**: main.py reorders shutdown cancellation so each background task logs its own crash reason rather than all of them racing. | P1 foundational | ⏳ this PR |
| **Stage-11-Step-B** | **Dual-currency top-up entry UI.** Wallet top-up screen gets a "USD / Toman" toggle (fa locale: "دلار / تومان"). When a user enters `400 000 تومان`, we convert via the live rate and *hold* the USD equivalent; the invoice carries the USD amount to NowPayments (unchanged pipeline). Entry-time rate is captured on the invoice so a rate shift between user-submit and gateway-settle doesn't screw anyone either way. `payments.invoice_amount_usd` stays the source of truth for credit; the Toman figure is purely UX. Free-text amount parsing tightens to accept fa digits and thousand-separators common in IR (`۴۰۰٬۰۰۰`, `400،000`, etc.). New pre-flight check: if the Toman amount converts to below the global $2 floor, refuse with the same "here's an alt" flow from Step-A. Strings (fa/en): 3 new keys — `topup_currency_prompt`, `topup_currency_toggle_toman`, `topup_currency_entered`. | P1 product | ⏳ pending |
| **Stage-11-Step-C** | **TetraPay (Rial card) gateway.** New module `tetrapay.py` with `create_order`, `verify`, and `handle_callback`. Create-order posts `{ApiKey, Hash_id, Amount, Description, Email, Mobile, CallbackURL}` to `https://tetra98.com/api/create_order`; on success (`status == "100"`) we get back `payment_url_web` + `Authority` + `tracking_id`. Webhook handler at `/tetrapay-webhook` accepts the callback (`{status, hash_id, authority}`) and double-verifies by calling `/api/verify` before crediting — do NOT trust the callback alone (same defensive pattern as NowPayments IPN). Credit amount is the USD equivalent locked **at invoice creation time**, NOT at settlement — otherwise a rate move between order-creation and callback robs the user. New DB migration 0011 adds `tetrapay_invoice_rate` col on the existing `invoices` table (or a `invoices_rates` sidecar, TBD — depends on which is less invasive to the NowPayments path). Secrets: `TETRAPAY_API_KEY` + `TETRAPAY_CALLBACK_BASE_URL` in `.env.example`. Replay / idempotency: dedupe on `authority` the same way we dedupe on NowPayments `payment_id`. Admin audit-log row per transition. | P1 correctness | ⏳ pending |
| **Stage-11-Step-D** | **Wallet display shows USD balance + live Toman equivalent.** `/wallet` and every balance surface renders `$2.00 (۳۴۰٬۰۰۰ تومان)` with the Toman figure computed from `fx_rates.get_usd_to_irr()` at render time. If the FX ticker is stale (e.g. the rate-source has been down for >1h), render a small "(approx)" marker next to the Toman figure so the user knows the number is last-known rather than live. Fa locale uses Persian digits for the Toman number, en locale uses Western digits — reuse the existing `strings.format_number_locale` helper. No DB change; this is display-layer only. **Bundled bug fix TBD.** | P2 product | ⏳ pending |
